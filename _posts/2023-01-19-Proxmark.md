...
Esto nos mostrará un monton de información de nuestra proxmark, y lo que mas no interesa en este caso es que en la parte de `HF` y en la de `LF` nos ponga `OK` porque eso significa que ambas antenas funcionan bien, en caso de que en una no ponga `OK` significa que esa antena no funciona.
